...
#define yaxis 40 //Map size y-axis
...
int snakeSize = 50, locX,locY;
int SleepDuration=100;  /*Added a new variable for */
...
void Grow (int coX, int coY) //for growing snake purpose, implementation of 'ADD TO END LIST'
{
    ...
    snakeSize++;
    SleepDuration -=1;           /*Every the time the snake grows the sleep time decreases so the snake can run faster each time   */
...
void Engine ()
{
    ...
    CurrScrn(); //REFRESH FRAME
        ...
        while(mapMove[locX][locY] != 'O') //IF HEAD DOES NOT OVERWRITE FOOD (eat), LOOP WILL NOT TERMINATE
        {
            ...
            if(Continuity == 0) //LOSING GAME WHEN CONTINUITY = 0
            {
                printRecord();
                return;
            }
